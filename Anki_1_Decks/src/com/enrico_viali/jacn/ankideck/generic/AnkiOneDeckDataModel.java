...
public class AnkiOneDeckDataModel   {
	...
	public boolean readCardModels(IAnkiDeckGeneric mgr) throws SQLException {
		...
		try {
			...
			{
				int nr = 0;
				queryStr = "SELECT id FROM cardModels";
				stFacts = dmdb.getConnection().createStatement();
				ResultSet cardsRs = stFacts.executeQuery(queryStr);
				while (cardsRs.next()) {
					nr++;
					long factID = cardsRs.getLong(1);
					cardModelIDs.add(factID);
					log.debug("letta cardModel entry " + ", ID: "
							+ cardsRs.getLong(1));
				}
			...
			{
				int nrItems = 0;
				for (Long id : cardModelIDs) {
					nrItems++;
					queryStr = "select id, name, qformat, aformat from cardModels where id = "
							+ id;
