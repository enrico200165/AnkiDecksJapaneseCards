 ...
 * 
 */
public class EVKanjHTMLRenderer implements ITexLineRenderer  {

	public EVKanjHTMLRenderer(String cssClassKanjiPar, String cssKanjiIDPar,
	...
	public String render(IRenderableAsTextLine ePar, long scanned, long included, int level) {
		...
		for (int i = 0; i < 5; i++) {
			if (e.getCompKanji(i) == null || e.getCompKanji(i).length() > 0) {
				...
				tr.addChild(td);
				// tdh.setWidth(300);
				//tr.addChild(tdh.cTD(null).addChild(SPAN.buildElement("compReading", "compReading", null, e.getCompKana(i))));
				// tdh.setWidth(400);
				//tr.addChild(tdh.cTD(null).addChild(SPAN.buildElement("compMean", "compMean", null, EdictUtils.noP(e.getCompMeaning(i)))));
	...
	private static org.apache.log4j.Logger log = Logger.getLogger(EVKanjHTMLRenderer.class);
}
